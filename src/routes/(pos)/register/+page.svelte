<script lang="ts">
	...
	import {
		...
		Minus,
		CircleCheck,
		Clock, // <-- Added the Clock icon for unpaid tabs
	} from "@lucide/svelte";
	...
	// We use this derived variable for the UI text (e.g., "Update Ticket #42")
	let editOrderId = $derived<number | null>(data.editOrderData?.id || null);

	// --- SVELTE 5 HYDRATION ROUTING FIX ---
	// We track the last loaded ID so we don't accidentally overwrite the cashier's
	// active work if SvelteKit runs a background data invalidation.
	...
	$effect(() => {
		...
		if (incomingId !== loadedEditId) {
			...
			if (incomingId && data.editOrderData) {
				// We loaded a new ticket to edit!
				// We use structuredClone to deep-copy the array so we aren't
				// mutating the raw SvelteKit data prop when we add/remove items.
				...
				// The URL was cleared (e.g., navigated back to a blank /register)
				cart = [];
				customerName = "";
				shift = "day";
				paymentMethod = "cash";
	...
	// 1. Extract unique categories for the tabs, sorted by category_id
	...
	// 2. Filter menu by Search Query AND Active Tab
	...
	// Reset amountTendered whenever the modal opens
	$effect(() => {
		if (showConfirmModal) {
			amountTendered = "";
		}
	...
	function handleProductClick(product: MenuItem) {
		const applicableGroups = data.modifierGroups.filter(
			(mg: ModifierGroup) =>
			...
			applicableGroups.forEach((group: ModifierGroup) => {
				if (group.min_selections >= 1 && group.max_selections === 1) {
					...
					if (groupOptions.length > 0) {
						chosenModifiers[groupOptions[0].id] = 1;
					}
				}
			});
			isModalOpen = true;
	...
	function updateCounterQty(
		...
		// If we are ADDING, we must check the group limit
		if (delta > 0 && maxSelections !== null) {
			...
			// Block the addition if the group is already at its max limit
	...
	function confirmConfiguration() {
		...
		const selectedMods: CartModifier[] = Object.entries(chosenModifiers)
			...
			.map(([idStr, qty]) => {
				...
				const modDef = data.modifiers.find((m: Modifier) => m.id === modId);
		...
		addItemToCart(selectedProduct, selectedMods);
		isModalOpen = false;
	}

	function addItemToCart(product: MenuItem, modifiers: CartModifier[]) {
		const modsPrice = modifiers.reduce((sum, m) => sum + m.price * m.qty, 0);
		const finalUnitPrice = Number(product.price) + modsPrice;
		...
		const cartId = `${product.id}-${modsString || "nomods"}`;

		const existing = cart.find((i) => i.cart_id === cartId);
		if (existing) {
			existing.qty += 1;
		} else {
			cart.push({
	...
	function handleCheckout() {
		...
		return async ({
			...
			if (result.type === "success") {
				...
				amountTendered = ""; // Reset cash calculator
...
<Dialog.Root bind:open={isModalOpen}>
	<Dialog.Content
		class="bg-card max-h-[90vh] overflow-y-auto border-2 border-dashed sm:max-w-100"
	>
		<Dialog.Header>
			<Dialog.Title class="font-black tracking-wide uppercase">Configure Order</Dialog.Title>
		...
		<Dialog.Footer>
			<Button onclick={confirmConfiguration} class="w-full font-bold tracking-wide uppercase">
				<Plus class="mr-2 h-4 w-4" /> Add to Order
			</Button>
		</Dialog.Footer>
	</Dialog.Content>
</Dialog.Root>
	...
	<Dialog.Content class="border-border sm:max-w-md">
		<Dialog.Header>
			...
			<Dialog.Description class="text-muted-foreground">
				Please review the ticket details before charging.
			</Dialog.Description>
		</Dialog.Header>
	...
	<div class="h-full w-full flex-1 lg:w-100">
		<Card class="border-border bg-card flex h-full flex-col border-2 py-0 shadow-lg">
			...
			<div class="flex-1 space-y-3 overflow-y-auto p-4">
				...
				{:else}
					{#each cart as item, i (item.cart_id)}
						<div
							class="bg-background border-border flex items-start gap-3 border p-3 shadow-sm"
						>
							<div
											...
											>
												...
												{#if mod.price > 0}
													<span class="opacity-60"
														>({formatMoney(mod.price * mod.qty)})</span
													>
												{/if}
											</div>
										{/each}
							...
							<div class="mt-1 text-right font-mono font-bold">
			...
			<div class="bg-secondary/10 border-border border-t p-6">
				...
				>
						...
						<div class="space-y-1">
							...
							<div class="bg-background border-border flex border p-1">
								<label class="flex-1 cursor-pointer">
									...
									/>
									<div
										class="hover:bg-secondary text-muted-foreground
										       peer-checked:bg-primary peer-checked:text-primary-foreground
											   flex h-9 items-center justify-center gap-1 rounded-sm p-2 text-xs font-bold uppercase transition-all"
									>
								...
								<label class="flex-1 cursor-pointer">
									...
									>
										...
										<span class="hidden xl:inline">QRIS</span>
									</div>
								</label>
								<label class="flex-1 cursor-pointer">
									<input
									...
									>
										<Clock class="h-4 w-4" />
										<span class="hidden xl:inline">Unpaid</span>
									</div>
								</label>
							</div>
						</div>
					...
					>
						{editOrderId ? `Update Ticket #${editOrderId}` : "Review Order"}
					</Button>
				</form>
			</div>
		</Card>
